...
if __name__ == '__main__':
    ...
    print('average logprob of a hawkes process whose parameters are generated by us (training)', prob_log/train_len)
    ...
    print('average logprob of a hawkes process whose parameters are generated by us (test)', prob_log/test_len)




    ## observing mu = 0, alpha = 1
    fun_len = 1000
    fun_data = [0]
    mu = 0.1
    alpha = 0.9
    for i in range(1, fun_len):
        val = predict(mu, alpha, History(fun_data[:i]))
        fun_data.append(val)
    
    plt.scatter(fun_data, np.ones(fun_len))
    plt.savefig("spec.png")
    


    # summed = log(times_delta_exp).sum(dim=1).unsqueeze(1)
    # alpha_term = (1-torch.exp(-1 * (self.final_T-self.times))).sum(dim=1).unsqueeze(1)
    # values = summed - alpha_term


    # np.log(np.exp((test_data[i] - History(curr).time_slots) * -1).sum()) - np.exp(-1 * (test_data[i] - curr[-1])) 
